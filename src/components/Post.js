...
export const Post = memo(
  ({
    ...
    const { username: currentUsername, id: userID } = useSelector(
    ...
    const reacting =
      useSelector((state) => state.posts.showReactions).filter((a) => a === id)
        .length > 0;
    const _owner = useSelector((state) =>
      state.users.find((a) => a.id === owner.id)
    );
    ...
    return (
      ...
      >
        <Topbar
          ...
          editing={{
            isEditable:
              tagged_persons?.length === 0 && multimedia?.length === 0,
            isEdited: hasEdits,
          }}
        ...
        />
        {(Array.isArray(multimedia) && multimedia?.length > 0) ||
        edits[0]?.text?.length > 0 ||
        content?.length > 0 ||
        ...
        {overlay ? <Overlay id={id}>{overlay}</Overlay> : null}
        ...
        {/* SHOW/HIDE REACTIONS */}
        {reacting ? <Reactions own={react?.ownReaction} id={id} /> : null}
